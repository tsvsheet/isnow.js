...
test("canonical: intervals render after the main form, before bounds", () => {
	...
	canon(
		"M-F +[90mn] >=6 <=18",
		"*/*/* Monday-Friday *:*:00 +[90mn] >=*/*/* * 06:00:00 <=*/*/* * 18:00:00",
	);
});
...
test("next / prev derive interval occurrences (civil-anchored)", () => {
	assert.equal(
		nextIso("+[25h]", "2026-07-14T00:00:00Z"),
		"2026-07-14T02:00:00.000Z",
	);
	assert.equal(
		nextIso("+[10d]", "2026-07-14T00:00:00Z"),
		"2026-07-21T00:00:00.000Z",
	);
	assert.equal(
		prevIso("+[10d]", "2026-07-21T00:00:01Z"),
		"2026-07-21T00:00:00.000Z",
	);
});
...
test("next / prev", () => {
	assert.equal(
		nextIso("6", "2026-07-14T07:00:00Z"),
		"2026-07-15T06:00:00.000Z",
	);
	assert.equal(
		nextIso("6", "2026-07-14T05:00:00Z"),
		"2026-07-14T06:00:00.000Z",
	);
	assert.equal(
		nextIso("12/25 midnight", "2026-06-01T00:00:00Z"),
		"2026-12-25T00:00:00.000Z",
	);
	assert.equal(
		nextIso("11/ Th-[1] noon", "2026-01-01T00:00:00Z"),
		"2026-11-26T12:00:00.000Z",
	);
	assert.equal(
		nextIso("F midnight", "2026-07-14T00:00:00Z"),
		"2026-07-17T00:00:00.000Z",
	);
	assert.equal(
		prevIso("6", "2026-07-14T07:00:00Z"),
		"2026-07-14T06:00:00.000Z",
	);
	assert.equal(
		prevIso("11/ Th-[1] noon", "2026-12-01T00:00:00Z"),
		"2026-11-26T12:00:00.000Z",
	);
});
...
test("next / prev honor and cap on bounds", () => {
	...
	assert.equal(
		nextIso("noon >=2027", "2026-01-01T00:00:00Z"),
...
test("zones: offset strings, IANA timeZone, and Date inputs", () => {
	...
	assert.equal(
		parse("noon", { timeZone: "America/New_York" }).holds(noonNy),
		true,
	);
	assert.equal(
		parse("noon", { timeZone: "America/New_York" }).holds(
	...
	assert.equal(
		p.next(new Date("2026-07-15T00:00:00Z")).toISOString(),
		"2026-07-15T16:00:00.000Z",
	);
	assert.equal(
		p.prev(new Date("2026-07-15T20:00:00Z")).toISOString(),
		"2026-07-15T16:00:00.000Z",
	);
});
...
test("api: is, toString, explain, error shape", () => {
	...
	assert.equal(
		parse("M,W,F noon").explain(),
		"holds at 12:00:00 on Monday,Wednesday,Friday",
	);
	assert.equal(
		parse("12 >=2011 <2016").explain(),
		"holds at 12:00:00 from 2011 until 2016",
	);
	assert.equal(
		parse("2026// *:*:00").explain(),
		"holds at *:*:00 in year 2026",
	);
	assert.equal(
		parse("*/*/15 4 12:00").explain(),
		"holds at 12:00:00 on Wednesday on day 15",
	);
...
test("errors: range", () => {
	const cases = [
		...
		":0+[90]",
		"0+[25]", // step stride must be < the field cycle
		"*/*/-40 *:*:00",
		"*/*/-0 *:*:00", // from-end tail must be 1..cycle (day 40 > 31, day 0 < 1)
		"Monday+[0] noon",
		"Monday+[6] noon", // weekday-occurrence index 1..5
		"*/*/+[99w] noon",
		"*/*/5+[3w] noon", // week stride 1..53 with anchor < stride
		":0+[99999999999999999999]",
		"/-99999999999999999999", // overflow magnitude fails every range check
		"M-F+[99] noon",
		"*/*/* 2-9-[1] 12:00", // setpos index 1..31 and a numeric endpoint out of the weekday domain
	];
