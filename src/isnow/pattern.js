...
import { newCtx, utcMs, withValue } from "./ctx.js";
...
export class Pattern {
	...
	_dayMatch(day, zone, fromMs, forward) {
		...
		return forward
			? this._firstAfter(insts, fromMs)
			: this._lastBefore(insts, fromMs);
	}
...
export function parse(src, options = {}) {
	...
	const canon =
		renderCanonical(sl, intervals, bounds) + renderExclusions(exclusions);
	return new Pattern(
		fields,
		bounds,
		intervals,
