...
const TIME_SYMBOLS = { midnight: [0, 0, 0], noon: [12, 0, 0], midday: [12, 0, 0] };
...
export function resolveWeekday(name) {
  ...
  if (Object.prototype.hasOwnProperty.call(WEEKDAY_RUNS, low)) {
...
export function resolveTime(name) {
  const low = name.toLowerCase();
  if (Object.prototype.hasOwnProperty.call(TIME_ABBREV, low)) {
...
function uniqueTime(hits) {
  ...
  if (hits.some((h) => h[0] !== hits[0][0] || h[1] !== hits[0][1] || h[2] !== hits[0][2])) {
